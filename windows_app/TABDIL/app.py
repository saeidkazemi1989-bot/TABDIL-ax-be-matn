...
class App(_BaseTk):
            ...
            try:
                ...
                if st["mode"] != "text":
                    ...
                    if _was_table:
                        ...
                        if cropped is not light:
                            light = cropped
                    xs_gray, ys_gray = detect_grid(gray0)
                    xs_light, ys_light = detect_grid(light)
                    if len(xs_gray) + len(ys_gray) > len(xs_light) + len(ys_light):
                        xs, ys = xs_gray, ys_gray
                        q.put(("status", f"grid از gray0: {len(xs)} عمودی، {len(ys)} افقی"))
                    else:
                        xs, ys = xs_light, ys_light
                        q.put(("status", f"grid از light: {len(xs)} عمودی، {len(ys)} افقی"))
                ...
                if kind == "table":
                    ...
                    try:
                        ...
                        def _is_poor_quality(w, c):
                            return (not w) or len(w) < 15 or c < 25
                        
                        best_words, best_conf = words, conf
                        
                        if _is_poor_quality(best_words, best_conf):
                            q.put(("status", f"کیفیت پایین ({len(best_words)} کلمه، {best_conf:.0f}%) - تلاش مجدد بدون حذف خطوط جدول..."))
                            try:
                                words2, conf2 = engine.recognize(light, lang=st["lang"], table_mode=True)
                                q.put(("status", f"بدون حذف خطوط: {len(words2)} کلمه، {conf2:.0f}%"))
                                if words2 and (len(words2) > len(best_words) or (len(words2) >= len(best_words)*0.8 and conf2 > best_conf)):
                                    best_words, best_conf = words2, conf2
                            except Exception as e:
                                q.put(("status", f"خطا در تلاش دوم: {e}"))
                        
                        if _is_poor_quality(best_words, best_conf):
                            q.put(("status", f"تلاش روی تصویر اصلی بدون بهبود..."))
                            try:
                                words3, conf3 = engine.recognize(gray0, lang=st["lang"], table_mode=True)
                                q.put(("status", f"روی تصویر اصلی: {len(words3)} کلمه، {conf3:.0f}%"))
                                if words3 and (len(words3) > len(best_words) or (len(words3) >= len(best_words)*0.8 and conf3 > best_conf)):
                                    best_words, best_conf = words3, conf3
                            except Exception as e:
                                q.put(("status", f"خطا در gray0: {e}"))
                        
                        if _is_poor_quality(best_words, best_conf):
                            q.put(("status", f"تلاش روی عکس خام بدون پردازش..."))
                            try:
                                words4, conf4 = engine.recognize(raw_original, lang=st["lang"], table_mode=True)
                                q.put(("status", f"عکس خام: {len(words4)} کلمه، {conf4:.0f}%"))
                                if words4 and (len(words4) > len(best_words) or conf4 > best_conf):
                                    best_words, best_conf = words4, conf4
                            except Exception as e:
                                q.put(("status", f"خطا در raw: {e}"))
                        
                        words, conf = best_words, best_conf
                        q.put(("status", f"نهایی: {len(words)} کلمه، اطمینان {conf:.0f}%"))
    ...
    def _build_table_editor(self, tab, idx, sh):
        try:
            ...
            if not rows:
                ...
                self.entry_grids[idx] = ("table", None, [], rows)
                return
            ...
            for c in range(max_cols):
                grid.grid_columnconfigure(c, weight=1, minsize=minsize)
                ...
                for c in range(max_cols):
                    ...
                    try:
                        e = ctk.CTkEntry(grid, 
                                         ...
                                         width=80 if max_cols > 10 else 100)
                        ...
                        try:
                            lbl = ctk.CTkLabel(grid, text=display_val[:50], font=ctk.CTkFont(FONT, 10), anchor="e",
                                               fg_color="#dbeafe" if r == 0 else "#ffffff")
                            lbl.grid(row=r, column=c, sticky="ew", padx=1, pady=1)
                            er.append(lbl)
                        except Exception as e2:
                            print(f"Label also failed r={r} c={c}: {e2}")
                            er.append(None)
                entries.append(er)
            ...
            if truncated:
                info_text += f" (نمایش {len(display_rows)} اول - بقیه در اکسل)"
            ...
            try:
                ...
                try:
                    self.entry_grids[idx] = ("table", None, [], sh.get("rows", []))
                except:
                    pass
    ...
    def _sync_active(self):
        """نوشتن مقادیر ویجت‌ها در داده‌ها - فیکس اکسل خالی."""
        for idx, sh in enumerate(self.sheets):
            ...
            if ed[0] == "text":
                ...
                try:
                    entries = ed[2]
                    has_full = len(ed) >= 4 and isinstance(ed[3], list)
                    full_rows = ed[3] if has_full else None
                    ...
                    for er in entries:
                        ...
                        for e in er:
                            try:
                                ...
                                row.append("")
                        rows_from_entries.append(row)
                    
                    if has_full and full_rows:
                        result_rows = []
                        for i in range(len(full_rows)):
                            if i < len(rows_from_entries):
                                edited_row = rows_from_entries[i]
                                original_row = full_rows[i]
                                if len(edited_row) < len(original_row):
                                    merged = edited_row + original_row[len(edited_row):]
                                else:
                                    merged = edited_row
                                result_rows.append(merged)
                            else:
                                result_rows.append(full_rows[i])
                        sh["rows"] = result_rows
                    else:
                        if rows_from_entries and all(all(not cell.strip() for cell in row) for row in rows_from_entries):
                            if has_full and full_rows:
                                sh["rows"] = full_rows
                            else:
                                sh["rows"] = rows_from_entries
                        else:
                            sh["rows"] = rows_from_entries
                except Exception as e:
    ...
    def save_outputs(self, prompt_after=False):
        ...
        if saved:
            names_list = [os.path.basename(p) for p in saved]
            names = "\n".join(names_list)
            self.status_lbl.configure(text="✓ ذخیره شد در: " + " | ".join(names_list))
    ...
    def quick_save_to_desktop(self):
        ...
        if saved:
            ...
            names = "\n".join(names_list)
            full_paths = "\n".join(saved)
            self.status_lbl.configure(text="✓ ذخیره شد روی دسکتاپ: " + " | ".join(names_list))
